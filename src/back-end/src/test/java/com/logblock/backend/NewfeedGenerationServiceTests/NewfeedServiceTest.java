...
public class NewfeedServiceTest {
    ...
    public void testGenerateFeedWithValidConnections() {
        ...
        List<Posting> postsFromUser2 = List.of(new Posting(101, "Post by User 2", 2));
        List<Posting> postsFromUser3 = List.of(new Posting(102, "Post by User 3", 3));
    ...
    public void testGenerateFeedWithNullConnections() {
        // Arrange: Trường hợp kết nối trả về null
    ...
    public void testGenerateFeedWithException() {
        ...
        when(connectionRepository.findByConnectorID(userID)).thenThrow(new RuntimeException("Database error"));

        // Act & Assert: Gọi phương thức và kiểm tra exception
        RuntimeException exception = assertThrows(RuntimeException.class, () -> newFeedService.generate(userID));
