...
public interface PostRepository extends JpaRepository<Posting, Integer> {
    ...
    /**
     * Retrieve posts sorted by number of upvotes within 2 weeks.
     *
     * @return List of trending votes
     */
    @Query("SELECT p FROM Posting p LEFT JOIN PostingUpvote up ON p.postID = up.postID GROUP BY p.postID ORDER BY COUNT(up.upvoterID) DESC")
    List<Posting> findTrendingPosts(Integer limit);
