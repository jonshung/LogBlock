...
function ProfileComponent({ name, bio, connectors }: { name: string, bio: string, connectors: number }) {
    const connected = 49;
                    ...
                    <div className="w-[600px] bg-white rounded-[15px]">
                        ...
                        <div className="px-[25px] py-[10px]">
                            {connectorCards.map((card, index) => {
                                return (
                                    <>
                                        <ConnectorsCard name={card.name} connectors={card.connectors} image={card.image} />
...
export default async function Page({ params }: { params: Promise<{ id: number }> }) {
    ...
    return (
        <main className="relative w-full top-[90px]">
